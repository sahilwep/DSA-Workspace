...
// Extra:
    - This problem is SAME as LC: 865
    - And Simillar to LC 236.
...
private:
    ...
    TreeNode* lca(TreeNode* node) {
        ...
        if(mp[node->val] == maxD) return node;

        TreeNode* left = lca(node->left);
        TreeNode* right = lca(node->right);

        if(left != nullptr && right != nullptr) return node;    // return current node as lca
        if(left != nullptr) return left;
        return right;
    }
public:
    TreeNode* lcaDeepestLeaves(TreeNode* root) {
        ...
        depth(root, 0); // find the depth for every node.
        return lca(root);
    }
};