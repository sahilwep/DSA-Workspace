...
public:
    int removeStones(vector<vector<int>>& stones) {
        ...
        // Finding the total number of components & generating answer:
        int totalComp = 0;
        for(int i = 0; i < (2 * (1e4 + 1)); i++) {  // iterate from 0 to the given range of dsu
            if(ds.parent[i] == i) { // get the ultimate parent
                if(ds.size[i] > 1) {    // not considering the single node components, because stones always have two nodes(row, col)
                    totalComp++;
                }
            }
        }
