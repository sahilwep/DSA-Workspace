...
//  Scrambled String Recursive Memoization Pruning
...
// Problem With Recursive Solution:
    * When we have a recursive solution with two or more decision points, there is a chance that the same subproblem will be computed multiple times across different branches of the recursive tree. 
    * The idea is to store the results of these redundant recursive calls so that when the same subproblem arises again, we can directly return the stored solution instead of repeating the recursive computation.



// Memoization Solution: 
    * We will use map data-structure to store the solution..
    * Map is more suitable here, because we are working with string, & we want to store the identifier as a string...
    * So, we will use map destructure:

            Key : Value
            String : int

    * Before solution the question, we will first check the solution into the map, if solution, exist then return directly, else compute & store in map, then return the value.
...
// Pruning Checks: 
    * During every recursive calls, we will check whether the both string has same frequency or not?
    * This will optimized on wasteful recursive calls for checking, distinct frequency string.
    * At every recursive calls, we will apply check for frequency..
    * As we have only 26 characters, we can create vector or size 26, & checks the frequency.
    * This will O(1), complexity for checking..



// Memoization Complexity:
    * TC: O(n^3)    -> Complexity remains same in worse case, but it's optimized over wasteful recursive calls.
...
#include<algorithm>
using namespace std;
unordered_map<string, bool> mp;     // Creating Map to store the solution of subproblems
...
bool isScramble(string s1, string s2){
    ...
    // Pruning Step: IF Character Frequencies differ, return false.
    vector<int> freq(26, 0);
    for(int i=0;i<s1.size();i++){
        freq[s1[i] - 'a']++;    // adding element
        freq[s2[i] - 'a']--;    // removing element
    }
    for(auto i : freq){
        if(i != 0) return false;    // if element frequency is not 0, we will return false..
    }


    // Checks for redundant subproblems:
    // Creating key:
    string key = s1 + ' ' + s2;     // Keys Creations 
    ...
    mp[key] = isScrambleFlag;   // Storing values in map, before returning
