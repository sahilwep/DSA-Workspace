...
//  Evaluate Expression to True Boolean Parenthesization Tabulation 
...
// Tabulation DP Solution: 
    * Converting recursive solution to tabulations is quite easy..
    * all we need to do is replace recursive pointer with iterations pointers.
    * Base case 1: 
        if (i > j) this will return 0
    * Base case 2: 
        if (i == j) i & j are equal, we will observe two different cases:

    * Case 1:(if we want the number of ways of true (i.e isTrue = 1)):
        if the single operand left is T(true), it will return 1 way, and if it's F(false), it will return 0 ways.
    
    * Case 2: (if we want the number of ways of false(i.e isTrue = 0)):
        If the single operand lef is T(true), it will return 0, ways and if it is F(false), it will return 1 ways.

    * In simple word: 
        * Replace "return" => "t[i][j][isTrue]"
        * replace i pointer & j pointer into loop...
...
using namespace std;
#define ll long long
#define mod 1003    // GFG Given Constrains 1003
...
int boolExp(string &s){
    int n = s.size();
    vector<vector<vector<ll> > > t(n + 1, vector<vector<ll> > (n+1, vector<ll> (2, 0)));    // create a 3D table to store the store the solution of subproblem.

    // First pointer i, start from last & comes upto start
    for(int i=n-1;i>=0;i--){
        // second pointer j, start from start & goes upto last.
        for(int j=0;j<=n-1;j++){
            
            // Base condition when i > j:
            if(i > j) continue;

            // third loop which compute the result based on the condition T/F
            for(int isTrue = 0; isTrue <= 1; isTrue++){
                ...
                for(int k=i+1;k<=j-1;k+=2){
                    ...
                    // Computing Finding final answer:
                    // Simillar logic, that we have done in recursive solution:
                    if(s[k] == '&'){
    ...
    return t[0][n-1][1];    // returning with (i = 0), (j = n-1), (isTrue = true)
}
...
// Recursive Solution:
int boolExp_rec(string &s, int i, int j, bool isTrue){
    // base case: 
    ...
    for(int k=i+1;k<=j-1;k=k+2){
        ...
        // Possible Value For left & right:
        // Left:  -> Left can be true or False, so we need to check for both the condition... 
        ll lT = boolExp_rec(s, i, k-1, 1);
        ll lF = boolExp_rec(s, i, k-1, 0);
        
        
        // right: -> right can be true or False, so we need to check for both the condition... 
        ll rT = boolExp_rec(s, k+1, j, 1);
        ll rF = boolExp_rec(s, k+1, j, 0);
    ...
    return ways;    // return the number of ways we can get True from the given expression..
}
...
void solve(){
    ...
    // Calling DP Tabulations Function:
    cout << boolExp(s) << endl;
