...
// Optimizations: 
    * We were solving Problem:  solve(e, f)
                 ___
                |   | F
                |   | . 
                |   | .  
    ...
    * In this problem, after dividing it into parts by a value 'k,' we further split it into two subproblems to solve individually. Initially, we assumed that neither of the subproblems had been evaluated, but what if they have been?
    * In this case, we should first check if both subproblems are already present in the memoization table. If they are, we can return their results immediately without entering recursive calls.
    * You might wonder why this is necessary, especially since we've already added checks for each subproblem in our code. However, reaching those checks requires storing the calls in the recursive stack, which adds unnecessary overhead. Therefore, it’s more efficient to check the memoization table first before proceeding with recursive calls and additional checks.
    

        We will replace this line:
            int tempAns = 1 + max(eggDrop(e-1, k-1, t), eggDrop(e, f-k, t));
        by:
            if(t[e-1][k-1] != -1){
                int low = t[e-1][k-1];
            }else{
                int low = eggDrop(e-1, k-1, t)
                t[e-1][k-1] = low;
            }
            if(t[e][f-k] != -1){
                int high = t[e][f-k];
            }else{
                high = eggDrop(e, f-k, t)
                t[e][f-k] = high;
            }

    This optimization won't effect much in time complexity...
...
int eggDrop(int e, int f, vector<vector<int> > &t){
    ...
    for(int k=1;k<=f;k++){  // Partition start from '1' & goes upto 'F' Floors.
        ...
        // Optimized over blind recursive calls:
        int low = (t[e-1][k-1] != -1) ? t[e-1][k-1] : t[e-1][k-1] = eggDrop(e-1, k-1, t);
        int high = (t[e][f-k] != -1) ? t[e][f-k] : t[e][f-k] = eggDrop(e, f-k, t);
