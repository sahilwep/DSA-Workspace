...
int evaluateExpression(string &s, int i, int j, bool isTrue, vector<vector<vector<ll > > > &t){
    // Base Condition:
    if(i > j) return 0;
    ...
    for(int k=i+1;k<=j-1;k=k+2){
        ll lT = evaluateExpression(s, i, k-1, 1, t);
        ll lF = evaluateExpression(s, i, k-1, 0, t);

        ll rT = evaluateExpression(s, k+1, j, 1, t);
        ll rF = evaluateExpression(s, k+1, j, 0, t);
...
void solve(){
    ...
    cout << evaluateExpression(s, 0, n-1, 1, t) << endl;
